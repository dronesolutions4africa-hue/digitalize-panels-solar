...
def pyramid_pooling_block(input_tensor, bin_sizes):
    ...
    # Derive spatial dims from the actual tensor shape so the block works for
    # any input resolution (256×256 → w=32, 512×512 → w=64, etc.)
    spatial = tf.keras.backend.int_shape(input_tensor)
    w = spatial[1]
    h = spatial[2]
